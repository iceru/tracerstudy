...
<div class="input-title">
    <h2>Input Pertanyaan Direct Answer / Text</h2>
    <hr>
</div>
...
<div class="input-form">
    <form action="{{ route('pertanyaan.storeDirect') }}" method="post">
        @csrf
    ...
    <table class="table">
        ...
        <tbody>
            ...
            <tr>
                ...
                <td>{{ $item->id }}</td>
                <td>{{ $item->nama_pertanyaan_da }}</td>
                <td id="action">
                    <a href="{{ route('pertanyaan.editDirect', $item->id) }}"><i class="fas fa-edit    "></i> Edit </a>
                    <form action="{{ route('pertanyaan.destroyDirect', $item->id )}}" method="get">
                        @csrf
                        @method('DELETE')
